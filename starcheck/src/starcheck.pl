...
my $chandra_models_version = call_python("calc_ccd_temps.get_chandra_models_version");
...
if ($mp_top_link) {
    $out .= sprintf("<A HREF=\"%s\">Short Term Schedule: %s</A>",
...
# The attitude checks are possible with either the characteristics file or the dynamic aimpoint file
    ...
    if ((not defined $char_file) and (not defined $aimpoint_file)) {
        ...
        if ($att_check->{att_check_ok} == 1) {
            ...
            $out .=
"<A HREF=\"${att_report}\">[${red_font_start}NOT OK${font_stop}] Coordinate mismatch or error.</A>\n";
        }
...
"         ------  $load_segments[$load_seg_idx]->{date}   OBC Load Segment Begins     $load_segments[$load_seg_idx]->{seg_id} \n";
    ...
    $out .= sprintf "<A HREF=\"#obsid$obs{$obsid}->{obsid}\">OBSID = %5s</A>",
      $obs{$obsid}->{obsid};
...
if ($par{html}) {
    open(my $OUT, "> $STARCHECK.html")
      or die "Couldn't open $STARCHECK.html for writing\n";

    #    print $OUT $ptf->ptf2any('html', $out);
    print $OUT
qq{<HTML><HEAD><script type="text/javascript" src="${STARCHECK}/overlib.js"></script></HEAD>};
    print $OUT
qq{<BODY><div id="overDiv" style="position:absolute; visibility:hidden; z-index:1000;"></div>$out</BODY></HTML>};
    close $OUT;
...
##***************************************************************************
    ...
    foreach (@lines) {
        ...
        my ($date) = split;
        my ($bs_obsid) =
          grep { $_->{date} eq $date and $_->{cmd} eq 'MP_OBSID' } @{$bs};
