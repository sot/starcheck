...
# Create a hash to store all information about the checks as they are performed
    ...
    my $tlr_file = get_file( "$par{dir}/$par{tlr}", 'tlr', 'required',
        \@{ $feedback{input_files} } );
    my $tlr = TLR->new( $tlr_file, 'tlr', \%config );
    $feedback{tlr} = $tlr;

#my $mm_file = get_file("$par{dir}/$par{mm}", 'Maneuver Management', 'required', \@{$feedback{input_files}});
    my $dot_file = get_file( "$par{dir}/$par{dot}", 'DOT', 'required',
        \@{ $feedback{input_files} } );

    #my @mm = Ska::Parse_CM_File::MM({file => $mm_file, ret_type => 'array'});
    my ( $dot_href, $s_touched, $dot_aref )
        = Ska::Parse_CM_File::DOT($dot_file);
    my $bs_file
        = get_file( "$par{dir}/$par{backstop}", 'Backstop', 'required',
        \@{ $feedback{input_files} } );
    ...
    my %templates = map {
        $_ => TLR->new(
            get_file(
                "$par{app_data}/$config{file}{template}{$_}",
                'template', 'required', \@{ $feedback{input_files} }
            ),
            'template',
            \%config
            )
    ...
    $feedback{aca_init_command} = compare_timingncommanding(
        [ $tlr->{first_aca_hw_cmd} ],
        [ $templates{A}->{entries}[0] ],
        \%config,
        "First ACA command is the template-independent init command"
    );
    %feedback = (
        %feedback, %{ replicas( $tlr, \%templates, $timelines, \%config ) }
    );

    $feedback{check_dither_enable_at_end}
        = check_dither_enable_at_end( $tlr, \%config );
    $feedback{check_dither_param_at_end}
        = check_dither_param_at_end( $tlr, \%config );
    $feedback{check_manvr} = check_manvr( \%config, $manvrs );
    $feedback{check_dwell} = check_dwell( \%config, $manvrs, $dwells );
    $feedback{check_manvr_point}
        = check_manvr_point( \%config, \@bs, $manvrs );
    $feedback{check_momentum_unloads}
        = check_momentum_unloads( \%config, \@bs, $manvrs, $dwells );
...
##***************************************************************************
    ...
    for my $manvr ( @{$manvrs} ) {
        if ( defined $dwell_start ) {
            $dwell{ $manvr->{init} } = {
                duration => $manvr->{tstart} - $dwell_start,
                tstart   => $dwell_start,
                tstop    => $manvr->{tstart},
                oflsid   => $manvr->{init},
            };
...
# note that begin_replica and end_replica use trace_ids from the TLR
        ...
        for my $entry (@trim_tlr) {
            if (    ( $entry->index() >= $r_start )
                and ( $entry->index() <= $r_end ) )
            {
...
# run the command checks on each transponder and store the results in the %per_trans hash
        ...
        for my $trans (qw( A B )) {
            ...
            for my $entry ( @{ $template->{entries} } ) {
                if (    ( defined $entry->replica() )
                    and ( $entry->replica() == $r_idx ) )
                {
                    push @replica_templ, $entry;
                }
            }
            $per_trans{$trans} = compare_timingncommanding(
                \@replica_tlr,
                \@replica_templ,
                $config,
...
# for the given transponder and replica, check that the transponder state is correct
        $feedback{"trans_replica_${r_idx}"} = check_iu(
            {   replica     => $r_idx,
        ...
        $feedback{"dither_disable_${r_idx}"}
            = check_dither_disable_before_replica( $tlr, $config, $r_idx );
    }
...
##***************************************************************************
    ...
    my %tmpl = (
        A => $config->{template}->{A}->{transponder},
    ...
    if ( not defined $state->{iu} ) {
        push @{ $output{info} },
            {
            text => "IU config not set (should be CIU512X or CIU512T)",
            type => 'error'
            };
        $output{status} = 0;
    }
        ...
        if ( $state->{iu} eq $want_iu ) {
            push @{ $output{info} },
                {
                text => "IU config set to $want_iu at " . $state->{datestart},
                type => 'info'
                };
        }
        else {
    ...
    if ( $state->{tstop} < $check_cfg->{r_tstop} ) {
        push @{ $output{info} },
            {
            text =>
                "Transponder state change during replica at $state->{datestop}",
            type => 'error'
            };
    }
    else {
        push @{ $output{info} },
            {
            text =>
                "Transponder state unchanged through replica end at $check_cfg->{r_datestop}",
            type => 'info'
            };
    }
    ...
    for my $cm ( keys %want ) {
        push @tinfo,
            {
            text => "transponder state $cm set to "
                . $state->{$cm}
                . ", should be "
                . $want{$cm},
    ...
    push @{ $output{info} },
        {
        text =>
            "Checking against config for intended transponder $transponder",
        type => 'info'
        };
    ...
    if ( $t_select == 0 ) {
        ...
        push @{ $output{info} },
            {
            text => "Transponder not correctly set for $transponder",
            type => 'error'
            };
    }
    else {
        push @{ $output{info} },
            {
            text => "Transponder correctly set to $transponder",
            type => 'info'
            };
    }
    ...
    for my $entry ( @{ $tlr->{entries} } ) {
        if ( defined $entry->comm_mnem() ) {
            if ( $entry->comm_mnem() eq 'CIMODESL' ) {
                ...
                if ((      ( not defined $timeline{iu} )
                        or ( $iu ne $timeline{iu} )
                    )
                    and ( $iu ne 'CIMODESL' )
                    )
                {
            ...
            if ( $entry->comm_mnem() =~ /^(C(TX|PA))(A|B)(ON|OF)/ ) {
                my $tkey = "$1$3";
                my $tval = $tog{$4};
                if (   ( not defined $timeline{$tkey} )
                    or ( $timeline{$tkey} ne $tval ) )
                {
                    $timeline{$tkey}     = $tval;
                    $timeline{datestart} = $entry->datestamp();
                    $timeline{time}      = $entry->time();
...
##***************************************************************************
    ...
    my %cmd_list = map { $_ => $config->{template}{independent}{$_} }
        (qw( dither_enable dither_disable ));

    my $index_0 = $tlr->begin_replica($replica)->index();

    my $time_before_replica_0
        = $config->{template}{time}{dtime_dither_disable_repl_0};

    push @{ $output{criteria} },
        sprintf("Steps back through the TLR from the replica start time");
    push @{ $output{criteria} },
        sprintf( "where replica $replica starts at "
            . $tlr->begin_replica($replica)->datestamp() );
    push @{ $output{criteria} },
        sprintf(
        "and looks for any dither enable or disable command mnemonics :");

    my $string_cmd_list;
    for my $comm_mnem ( map {@$_}
        @{ cmd_list { 'dither_enable', 'dither_disable' } } )
    {
    ...
    for my $tlr_entry ( reverse @tlr_arr[ 0 .. $index_0 ] ) {
        next
            unless ( grep { $tlr_entry->comm_mnem() eq $_->{comm_mnem} }
            ( map {@$_} @{ cmd_list { 'dither_enable', 'dither_disable' } } )
            );
    ...
    if ( scalar(@last_dith_cmd) == 0 ) {
        push @{ $output{info} },
            {
            text => "No Dither Commands found before replica $replica",
            type => 'error'
            };
        $output{status} = 0;
        return \%output;
    ...
    push @{ $output{criteria} },
        "Compares discovered dither command to correct dither disable command";

    # confirm that the most recent dither command is correct
    my $match
        = entry_arrays_match( \@last_dith_cmd, $cmd_list{dither_disable} );
...
##***************************************************************************
    ...
    my %output = ( comment => ['Dither enabled after Dark Cal'], );
    ...
    push @{ $output{criteria} },
        sprintf(
        "Steps through the TLR from the last hw command at replica 4");
    push @{ $output{criteria} },
        sprintf("to the maneuver from DFC to ADJCT, where");
    push @{ $output{criteria} },
        sprintf( "end replica 4 at " . $tlr->end_replica(4)->datestamp() );
    push @{ $output{criteria} },
        sprintf( "maneuver away from dfc at  "
            . $tlr->manvr_away_from_dfc->datestamp() );
    push @{ $output{criteria} },
        sprintf("looks for any dither enable or disable command mnemonics :");
...
# and before the maneuver away from dark field center at the end of the calibration
    ...
    for my $tlr_entry (
        @tlr_arr[ $index_end .. $manvr_away_from_dfc->index() ] )
    {
        next
            unless ( grep { $tlr_entry->comm_mnem() eq $_->{comm_mnem} }
            ( map {@$_} @{ cmd_list { 'dither_enable', 'dither_disable' } } )
    ...
    push @{ $output{criteria} },
        "Throws error on 0 or more than 1 dither enable command in the interval";

    if ( scalar(@dith_cmd) == 0 or scalar(@dith_cmd) > 1 ) {
        if ( scalar(@dith_cmd) > 1 ) {
            push @{ $output{info} },
                { text => "Extra dither commands at end", type => 'error' };
            for my $entry (@dith_cmd) {
                push @{ $output{info} },
                    {
                    text => sprintf(
                        $entry->datestamp() . "\t" . $entry->comm_mnem()
                    ),
                    type => 'error'
                    };
            }
            ...
            push @{ $output{info} },
                { text => "Dither not enabled at end", type => 'error' };
        }
    ...
    push @{ $output{criteria} },
        "Compares discovered dither command to correct dither enable command";
...
##***************************************************************************
    ...
    my %output = ( comment => ['Dither param before Dark Cal'], );

    my @tlr_arr = @{ $tlr->{entries} };

    my $dither_null_param
        = $config->{template}{independent}{dither_null_param};
    ...
    push @{ $output{criteria} },
        sprintf("Steps back through the TLR from the replica $replica");
    push @{ $output{criteria} },
        sprintf( "where replica $replica at "
            . $tlr->begin_replica($replica)->datestamp() );
    push @{ $output{criteria} },
        sprintf("looks for any dither parameter command mnemonics :");
    ...
    push @{ $output{criteria} },
        "If no commands are found, generates an error.";

    if ( scalar(@last_dith_param) == 0 ) {

        push @{ $output{info} },
            {
            text => "Dither parameters not set before replica $replica",
            type => 'error'
            };
        $output{status} = 0;
        return \%output;
    ...
    push @{ $output{criteria} },
        "Otherwise, goes on and compares dither parameter command to correct dither parameter command";
...
##***************************************************************************
    ...
    my %output = ( comment => ['Dither param set to default at end'], );

    my @tlr_arr = @{ $tlr->{entries} };

    my $dither_default_param
        = $config->{template}{independent}{dither_default_param};
    ...
    push @{ $output{criteria} },
        sprintf(
        "Steps through the TLR from the last command at the end of replica 4"
        );
    push @{ $output{criteria} },
        sprintf("to the command to maneuver away from DFC to ADJCT");
    push @{ $output{criteria} },
        sprintf( "end of replica 4 at " . $tlr->end_replica(4)->datestamp() );
    push @{ $output{criteria} },
        sprintf( "maneuver away from dfc at "
            . $tlr->manvr_away_from_dfc->datestamp() );
    push @{ $output{criteria} },
    ...
    push @{ $output{criteria} },
        "Throws error on 0 or more than 1 dither parameter command in the interval";
    ...
    if ( scalar(@dith_param) == 0 or scalar(@dith_param) > 1 ) {
        ...
        if ( scalar(@dith_param) > 1 ) {
            push @{ $output{info} },
                { text => "Extra dither params set at end", type => 'error' };
            ...
            push @{ $output{info} },
                {
                text => "Default dither params not set at end",
                type => 'error'
                };
        }
...
# Check that the maneuver timing is consistent with the template for maneuvers to and from replicas
    ...
    push @{ $output{criteria} },
        "Compares the DC maneuver times to and from replicas to the config file times";
        ...
        if ( ( $manvr->{init} =~ /DC_T/ ) or ( $manvr->{final} =~ /DC_T/ ) ) {
            ...
            if ( not defined $expected_time ) {
                push @{ $output{info} },
                    {
                    text =>
                        "Illegal Dark Cal Maneuver. Unexpected maneuver from "
            ...
            push @{ $output{info} },
                {
                text => "Maneuver from $manvr->{init} to $manvr->{final} : "
    ...
    if ( ( grep { !/1/ } values %init ) or ( grep { !/1/ } values %final ) ) {
        push @{ $output{info} },
            {
            text =>
                "Extra maneuvering to or from replicas... check manually.",
            type => "error"
            };
        $output{status} = 0;
    }
...
##***************************************************************************
    ...
    push @{ $output{criteria} },
        "Checks the dwell time at every replica and at every Dark Field Center just before a replica.";
    ...
    for my $id (@check_ids) {
        if ( not defined $dwells->{$id} ) {
            push @{ $output{info} },
                {
                text => "No dwell found for $id from DOT maneuvers",
                type => "error"
                };
            $output{status} = 0;
        }
        my $template_type = ( $id =~ /DFC/ ) ? 'center' : 'replica';
        my ( $outputs, $status ) = single_dwell_check(
            $id,
            $dwells->{$id}->{duration},
...
##***************************************************************************
    ...
    push @outputs,
        {
        text =>
            "Dwell at $oflsid : time = $dwell secs  ; expected time = $expected_dwell secs",
        type => 'info'
        };
    ...
    if ( abs( $dwell - $expected_dwell ) > $dwell_tolerance ) {
        if ( $dwell < $expected_dwell ) {
            push @outputs,
                { text => "Dwell Time incorrect", type => 'error' };
            $status = 0;
        }
        else {
            push @outputs,
                {
                text => "Dwell Time too long (probably OK)",
                type => 'warn'
                };
            $status = 0;
        }
...
##***************************************************************************
    my $timestring = shift;
    my %timehash;
    ( $timehash{days}, $timehash{hours}, $timehash{min}, $timehash{sec} )
        = split( ":", $timestring );
    my $secs = 0;
...
##***************************************************************************
    ...
    for my $entry ( @{$bs} ) {
        if (    ( defined $entry->{command} )
            and ( defined $entry->{command}->{TLMSID} ) )
        {
    ...
    push @{ $output{criteria} },
        "Confirms no momentum dumps at DFC or at replicas or during manvr to or from replicas";
    ...
    for my $manvr ( @{$manvrs} ) {
        ...
        if ( ( $manvr->{init} =~ /DC_T/ ) or ( $manvr->{final} =~ /DC_T/ ) ) {
            push @{ $output{info} },
                {
                text => sprintf(
                    "Checking for momentum dumps during maneuver from %s to %s",
                    $manvr->{init}, $manvr->{final}
                ),
                type => 'info'
                };
            ...
            for my $unload (@unloads) {
                    ...
                    and ( date2time($unload) <= $manvr->{tstop} ) )
                {
                    ...
                    push @{ $output{info} },
                        {
                        text => sprintf(
                            "Momentum dump at %s during maneuver from %s to %s",
                            $unload, $manvr->{init}, $manvr->{final}
                        ),
                        type => 'error'
                        };
                }
    ...
    for my $id (@oflsids) {
        my ( $outputs, $status )
            = single_unload_check( $dwells->{$id}, \@unloads );
        push @{ $output{info} }, @{$outputs};
        $output{status} = 0 if ( $status == 0 );
    }
...
##***************************************************************************
    ...
    push @outputs,
        {
        text => sprintf(
            "Checking for unloads during $dwell->{oflsid} " . ": %s to %s ",
    ...
    for my $unload ( @{$unloads} ) {
            ...
            and ( date2time($unload) <= $dwell->{tstop} ) )
        {
            ...
            push @outputs,
                {
                text => sprintf(
                    "Momentum dump at %s during dwell at %s",
                    $unload, $dwell->{oflsid}
                ),
                type => 'error'
                };
...
##***************************************************************************
    ...
    push @{ $output{criteria} },
        "Confirms that the delta positions for each of the dark cal pointings",
...
# time
        ...
        my $targ_quat = Quat->new(
            $bs_match->{command}->{Q1}, $bs_match->{command}->{Q2},
            $bs_match->{command}->{Q3}, $bs_match->{command}->{Q4}
        );
...
# DFC/DC_T that are being checked.
        ...
        if ( $dest_obsid =~ /DC_T/ ) {
            $temp_delta->{q}[1]
                = arcsec_to_rad( $pointings{$dest_obsid}->{dx} ) / 2;
            $temp_delta->{q}[2]
                = arcsec_to_rad( $pointings{$dest_obsid}->{dy} ) / 2;
            $pred_point = $temp_delta->multiply($center_quat);
        }
        ...
        if ( !quat_near( $target{quat}, $pred_point, $as_slop ) ) {
            ...
            push @{ $output{info} },
                ...
                text => sprintf(
                    "Delta position of "
                        . $target{obsid}
                        . " relative to DFC_I is (% 7.2f, % 7.2f) : Correct",
                    $x, $y
                ),
                type => 'info'
                };
...
##***************************************************************************
    ...
    return acos(
        cos($d1) * cos($d2) * cos( ( $a1 - $a2 ) ) + sin($d1) * sin($d2) );
}
...
##***************************************************************************
    ...
    if ( scalar( @{$entries} ) != scalar( @{$config_entries} ) ) {
        $output{status} = 0;
        $output{info}   = [
            { text => "Mismatch in number of commands", type => 'error' } ];
        return \%output;
    }
...
##***************************************************************************
    ...
    push @{ $output{criteria} },
        "Compares TLR entries to template TLR entries";
    my $string_cmd_dict = join( " ", ( keys %command_dict ) );
    push @{ $output{criteria} }, $string_cmd_dict;

    push @{ $output{criteria} },
        "Checks each entry against template entry for matching timing, comm_mnem, and hex.";
    ...
    if ( scalar(@match_tlr_arr) < scalar( @{$templ_arr} ) ) {
        push @{ $output{info} },
            {
            text => "Not enough entries in the ACA commanding section",
            type => 'error'
            };
        $output{status} = 0;
    }
...
##***************************************************************************
    ...
    $out .= sprintf(
        "["
            . is_ok(
                    $dark_cal_checker->{trans_replica_0}->{status}
                ...
                and $dark_cal_checker->{trans_replica_4}->{status}
            )
    ...
    $out .= sprintf(
        "["
            . is_ok(
                    $dark_cal_checker->{tnc_replica_0}->{status}
    ...
    $out .= sprintf(
        "["
            . is_ok(
                    $dark_cal_checker->{check_manvr}->{status}
                and $dark_cal_checker->{check_dwell}->{status}
            )
            . "]\tManeuver and Dwell timing.\n"
    );
    $out
        .= sprintf( "["
            . is_ok( $dark_cal_checker->{check_manvr_point}->{status} )
            . "]\tManeuver targets.\n" );
    $out .= sprintf(
        "["
            . is_ok(
                    $dark_cal_checker->{dither_disable_0}->{status}
                ...
                and $dark_cal_checker->{check_dither_param_at_end}->{status}
            )
            . "]\tDither enable/disable and parameter commands\n"
    );
    ...
    for my $t ( 0 .. 4 ) {
        if ( $trans ne $self->{"tnc_replica_$t"}->{'transponder'} ) {
            ...
            if ( $t > 0 ) {
                $text
                    .= "\tafter "
                    . $self->{'tlr'}->end_replica( $t - 1 )->datestamp()
                    . "\n\t";
            }
            $text .= "\tbefore "
                . $self->{'tlr'}->begin_replica($t)->datestamp() . "\n";
...
##***************************************************************************
    ...
    my $opt
        = 1 == @_
        ? pop @_
        : { 'criteria' => 0, 'verbose' => 0, 'html_standalone' => 0 };
    ...
    if ( !$opt->{criteria} & !$opt->{verbose} & !$opt->{html_standalone}
        & defined $opt->{link_to} )
    {
    ...
    if ( $opt->{verbose} ) {
        for my $entry ( @{ $feedback->{info} } ) {
            ...
            if ( $type eq 'error' ) {
                $return_string
                    .= "${red_font_start} --->>>  $line${font_stop}\n";
            }
            if ( $type eq 'warn' ) {
                $return_string
                    .= "${yellow_font_start} --->>>  $line${font_stop}\n";
            }
        }
...
##***************************************************************************
    ...
    croak("No ACA commanding found.")
        unless defined $self->{first_aca_hw_cmd};
...
##***************************************************************************
    ...
    return $self->{begin_replica}->{$replica}
        if ( defined $self->{begin_replica}->{$replica} );
    ...
    for my $entry ( @{ $self->{entries} } ) {
        ...
        if (    ( defined $entry->trace_id() )
            and ( $entry->trace_id() =~ /ADC_R$replica/ ) )
        {
...
##***************************************************************************
    ...
    return $self->{end_replica}->{$replica}
        if ( defined $self->{end_replica}->{$replica} );
...
##***************************************************************************
    ...
    return $self->{manvr_away_from_dfc}
        if ( defined $self->{manvr_away_from_dfc} );
...
##***************************************************************************
    ...
    for my $line_index ( 0 .. $#raw_tlr_array ) {
        ...
        my $timestamp
            = tlr_substr( $raw_tlr_array[$line_index], $field->{datestamp} );

        if ( has_timestamp($timestamp) ) {

            my $hex = tlr_substr( $raw_tlr_array[$line_index],
                $arr_field->{hex} );
            ...
            if ( has_hex($hex) ) {
                ...
                for my $key ( keys %{$field} ) {
                    $linehash{$key} = tlr_substr( $raw_tlr_array[$line_index],
                        $field->{$key} );
                }
...
##***************************************************************************
    ...
    for my $line_index ( 0 .. $#raw_tlr_array ) {
        ...
        if ( has_timestamp($timestamp_area) ) {
            my $hex_area = tlr_substr( $raw_tlr_array[$line_index],
                $arr_field->{hex} );
...
#	push @{$output{error}} ,sprintf($entry1->datestamp() . "\t" . $entry1->comm_mnem() . "\t" . $entry1->comm_desc());
    ...
    my $step_rel_time_match = (
        abs(      $entry1->step_rel_time_replica()
                - $entry2->step_rel_time_replica()
        ) < $REL_TIME_TOL
    );
...
#	push @{$output{error}}, sprintf("step relative time mismatch: " . $entry1->step_rel_time_replica() . " secs tlr, " . $entry2->step_rel_time_replica() . " secs template ");
        ...
        push @{ $output{info} },
            {
            text => sprintf(
                      "step relative time mismatch: "
                    . $entry1->step_rel_time_replica()
                    . " secs tlr, "
                    . $entry2->step_rel_time_replica()
                    . " secs template "
            ),
...
#    }
    ...
    if (    ( $entry1->comm_mnem() eq $entry2->comm_mnem() )
        and ($step_rel_time_match)
        and ( $hex_equal->{status} ) )
    {
        $output{status} = 1;
    }
...
##***************************************************************************
    ...
    if ( scalar( @{$hex_a} ) != scalar( @{$hex_b} ) ) {
        $output{info} = [
            {   text => "hex commands have different number of entries",
                type => 'error'
            }
        ];
        $output{status} = 0;
    ...
    for my $i ( 0 .. scalar( @{$hex_a} ) - 1 ) {
        if ( $hex_a->[$i] ne $hex_b->[$i] ) {
            $output{info} = [
                {   text =>
                        "\tBad hex: $hex_a->[$i] does not match expected $hex_b->[$i]",
                    type => 'error'
                }
            ];
            $output{status} = 0;
        ...
        push @{ $output{info} },
            {
            text => "\thex ok: $hex_a->[$i] matches expected $hex_b->[$i]",
            type => 'info'
            };
    }
